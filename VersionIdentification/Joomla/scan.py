...
def full_scan(hostinfo, init_tree, probe_data, ctype="joomla",auth_flag:str=None):

    filter_versions = ['']
    versions_file = 'docker_versions.txt'
...
idx = 0
reshost=[]
def scan_host(hostinfo, save_file_path, offline_file_path, probe_data, ctype, auth_flag):
    try:
        off_fp = os.path.join(offline_file_path, hostinfo.split("://")[0], hostinfo.split("://")[1].replace("/","_"))  
        if os.path.exists(off_fp):
            return None  

        base_dir = os.path.join(save_file_path, hostinfo.split("://")[0], hostinfo.split("://")[1].replace("/","_") + "/result.json")
        if os.path.exists(base_dir):
            return None  

        # res = requests.get(hostinfo, timeout=10,verify=False)  
        # if res.status_code == 200:
        res = subprocess.run(
            ['curl', '-s', '-o', '/dev/null', '-w', '%{http_code}', '--insecure', hostinfo],
        ...
        status_code = res.stdout.decode().strip()

        if status_code == '200':
            try:

                new_probe_data = {key: [set(v_list) for v_list in probe_data[key]] for key in probe_data}
                ...
                base_dir = os.path.join(save_file_path,hostinfo.split("://")[0], hostinfo.split("://")[1].replace("/","_"))
                ...
                if len(results) > 2 and len(results[2]) > 0:
                    results[2] = sorted(results[2], key=lambda x: tuple(map(int, x.split("."))))
                    print(results)
            ...
            off_fp = os.path.join(offline_file_path, hostinfo.split("://")[0], hostinfo.split("://")[1].replace("/","_"))
            os.makedirs(off_fp, exist_ok=True)
        ...
        off_fp = os.path.join(offline_file_path, hostinfo.split("://")[0], hostinfo.split("://")[1].replace("/","_"))
        os.makedirs(off_fp, exist_ok=True)
        print(f"{hostinfo} request timed out")
        return hostinfo
    except Exception as e:
        print(f"Request failed for {hostinfo}: {e}")
        traceback.print_exc()
        return None


def worker(hostinfo, save_file_path, offline_file_path, probe_data, ctype, auth_flag, not_exist_hosts, lock):
    result = scan_host(hostinfo, save_file_path, offline_file_path, probe_data, ctype, auth_flag)
    if result:
        
        with lock:
            not_exist_hosts.append(result)
...
def plain_job(debug=False):
    ...
    probe_data = get_data_from_json("VersionSeek/Probes/Joomla/joomla_probe_data.json")
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        ...
        for one in multi_hosts:
            hostinfo = one.strip()
            future = executor.submit(worker, hostinfo, save_file_path, offline_file_path, probe_data, ctype, auth_flag, not_exist_hosts,lock)
    ...
    end_time = time.time()
    print((end_time - start_time) / len(multi_hosts))
    #print(reshost)



def replan(hostinfo, ctype="joomla", debug=True, limit_probe=[]):
    ...
    try:    
        ...
        if not os.path.exists(base_dir):
            os.makedirs(base_dir)
        results = full_scan(hostinfo, init_tree, new_probe_data, ctype)
        if len(results) > 2 and len(results[2]) > 0:
            results[2] = sorted(
...
def get_average_time(data:dict):
    with open("", 'r') as file:
        #for line in file:
        multi_hosts=file.readlines()
    ...
    for one in multi_hosts:    
        ...
        if idx > number:
            break
        one1 = json.loads(one.strip())
        protocol = one1.get('protocol', '')
        host = one1.get('host', '')
        
        if protocol and host:
            if not host.startswith("http"):
                hostinfo = f"{protocol}://{host}"
            else:
                hostinfo = host
        if len(hosts) < number:
        ...
        print(f"Scanning {30} servers using time {total_time:.6f} seconds, average time is {average_time:.6f} seconds")
    else:
